...
>
> 而後端程式在驗證登入時，正常的語法只有一條 `SELECT * FROM users WHERE username = '<輸入>' AND password = '<輸入>';`，中間分兩句的話，相當於強迫資料庫在原本預期一條指令的管道中執行了多條語句、堆疊語句 (stacked queries)，許多後端網頁驅動程式預設在內部看到分號，為了安全起見就會報錯。